...
resource "aws_s3_bucket" "datawarehouse" {
  bucket = "datawarehouse-dados"
  tags = {
    Name        = "datawarehouse"
    Environment = "Dev"
    Managedby   = "Terraform"
...
resource "null_resource" "upload_spark_script" {
  provisioner "local-exec" {
    command = "aws s3 cp src/spark/transformed_data.py s3://${aws_s3_bucket.processing-job-py.bucket}/scripts/transformed_data.py"
  }
}